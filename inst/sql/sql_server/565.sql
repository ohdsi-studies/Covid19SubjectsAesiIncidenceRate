...
WHERE (C.measurement_date >= DATEFROMPARTS(2019, 12, 1) and C.measurement_date <= DATEFROMPARTS(2020, 12, 1))
...
WHERE (C.condition_start_date >= DATEFROMPARTS(2019, 12, 1) and C.condition_start_date <= DATEFROMPARTS(2020, 12, 1))
...
) A on A.person_id = P.person_id  AND A.START_DATE <= DATEADD(day,3,P.START_DATE) ) cc on p.person_id = cc.person_id and p.event_id = cc.event_id
GROUP BY p.person_id, p.event_id
HAVING COUNT(cc.event_id) = 0
-- End Correlated Criteria
